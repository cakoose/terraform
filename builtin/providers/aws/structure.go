...
//     "bar=12"
//
// In Terraform config, there are two differences:
// - Use a list of strings instead of separating strings with newlines.
// - Within each string, don't include the surrounding quotes.
// Example:
//     records = ["x=foo", "bar=12"]
//
...
//     "ba" "r" "=12"
//
// Since `expandTxtEntry` only removes the quotes at the end we can still
// represent the above configuration in Terraform:
//      records = ["x=\" \"foo", "ba\" \"r\" \"=12"]
//
// The reason to use multiple strings for a single entry is that Route 53 (and
// DNS in general) doesn't allow a quoted string to be more than 255 characters.
// To create an entry that is longer, you must use multiple quoted strings.  DNS
// clients are expected to merge them.
//
